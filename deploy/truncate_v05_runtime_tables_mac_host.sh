...
SCENARIO="all"
PRESERVE_ML_AI="true"
PRESERVE_BASELINE_REFERENCE="true"
PRESERVE_OBS_BASELINE_REFERENCE="${PRESERVE_OBS_BASELINE_REFERENCE:-true}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --preserve-baseline-reference) PRESERVE_BASELINE_REFERENCE="$2"; shift 2 ;;
    --preserve-obs-baseline-reference) PRESERVE_OBS_BASELINE_REFERENCE="$2"; shift 2 ;;
...
echo "[TRUNCATE] preserve_ml_ai=$PRESERVE_ML_AI preserve_baseline_reference=$PRESERVE_BASELINE_REFERENCE remove_source_files=$REMOVE_SOURCE_FILES dry_run=$DRY_RUN"


if [[ "$PRESERVE_BASELINE_REFERENCE" == "false" || "$PRESERVE_BASELINE_REFERENCE" == "0" ]]; then
  echo "[TRUNCATE] remove Baseline Reference rows for selected date scope"
  for t in v05_baseline_metric_snapshot_day v05_baseline_reference_run_day v05_baseline_distribution_snapshot_day; do
    delete_scoped "$t"
  done
else
  echo "[TRUNCATE] preserve Baseline Reference tables"
fi

if [[ "$PRESERVE_OBS_BASELINE_REFERENCE" == "false" || "$PRESERVE_OBS_BASELINE_REFERENCE" == "0" || "$PRESERVE_BASELINE_REFERENCE" == "false" || "$PRESERVE_BASELINE_REFERENCE" == "0" ]]; then
  echo "[TRUNCATE] remove OBS Baseline Reference rows for selected date scope"
  delete_scoped v05_obs_baseline_reference_day
else
  echo "[TRUNCATE] preserve OBS Baseline Reference table"
fi
...
for t in \
  ...
  reliability_analysis_result_day_v05 v05_cross_domain_propagation_evidence_day v05_reconciliation_measurement_day v05_runtime_evidence_day v05_semantic_action_calibration_review_day \
  v05_observability_measurement_day r_v05_observability_analysis_day r_v05_observability_interpretation_day v05_observability_anomaly_trace_day \
  ...
  v05_batch_score_contribution_day v05_batch_behavior_anomaly_day v05_batch_metric_delta_day \
